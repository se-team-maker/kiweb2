...
require_once __DIR__ . '/teachers-bootstrap.php';
...
SQL;
    ...
    error_log('[user-sync] DB query failed: ' . $exception->getMessage());
    $payload = [
        'success' => false,
        'error' => 'Internal server error',
        'error_code' => 'DB_QUERY_FAILED',
    ];
    if (!empty($_ENV['USER_SYNC_DEBUG'])) {
        $payload['debug'] = $exception->getMessage();
    }
    ts_json_response($payload, 500);
} catch (RuntimeException $exception) {
    error_log('[user-sync] Bootstrap failed: ' . $exception->getMessage());
    ...
    error_log('[user-sync] Unexpected error: ' . $exception->getMessage());
    ts_json_response([
        'success' => false,
        'error' => 'Internal server error',
