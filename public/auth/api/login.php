...
try {
    ...
    error_log('Password login failed: ' . $exception->getMessage());
    errorResponse('ログイン処理に失敗しました。DB設定を確認してください。', 500);
}
...
if (is_array($user) && isset($user['error']) && $user['error'] === 'email_not_verified') {
    ...
    $redirectUrl = '/kiweb/public/auth/verify-email.php?id=' . $tokenData['token_id']
        . ($remember ? '&remember=1' : '');
