...
// API エンドポイント
const API = {
    users: '/kiweb/public/auth/api/admin/users.php',
    userRoles: '/kiweb/public/auth/api/admin/user-roles.php',
    userScopes: '/kiweb/public/auth/api/admin/user-scopes.php',
    roles: '/kiweb/public/auth/api/admin/roles.php',
    rolePermissions: '/kiweb/public/auth/api/admin/role-permissions.php',
    permissions: '/kiweb/public/auth/api/admin/permissions.php',
    scopes: '/kiweb/public/auth/api/admin/scopes.php',
    scopeTypes: '/kiweb/public/auth/api/admin/scope-types.php',
    accessLogs: '/kiweb/public/auth/api/admin/access-logs.php'
};
...
async function loadUsers() {
    ...
    try {
        ...
        if (error.message && (error.message.includes('認証') || error.message.includes('401'))) {
            ...
            setTimeout(() => {
                window.location.href = '/kiweb/public/auth/login.php';
            }, 2000);
            return;
        }
...
function getAccessLogFilters() {
    return {
        ...
        event_type: (document.getElementById('log-event-type')?.value || '').trim(),
        page_key: (document.getElementById('log-page-key')?.value || '').trim(),
...
async function loadAccessLogs(page = 1) {
    ...
    if (filters.page_key) params.set('page_key', filters.page_key);
...
function clearAccessLogFilters() {
    const ids = ['log-date-from', 'log-date-to', 'log-user-search', 'log-page-path', 'log-event-type', 'log-page-key'];
    ids.forEach((id) => {
...
function renderAccessLogRows(items) {
    ...
    if (!Array.isArray(items) || items.length === 0) {
        tbody.innerHTML = `
            <tr>
                <td colspan="8" class="access-log-empty-cell">対象データがありません。</td>
            </tr>
        `;
        return;
    }
    ...
    tbody.innerHTML = items.map((item) => {
        ...
        const eventLabel = eventType === 'iframe_open' ? 'iframe表示' : 'ページ表示';
        const pageLabel = item.page_label || item.page_key || '-';
