 ...
 *
 * この部品は各ポータルへ表示を追加する共通UIであり、ログインや権限管理は行わない。
 * 呼び出し元は認証完了後にログイン本人名を渡す。検索画面で一時的に選んだ講師名を
 * タスク担当者として使わないことが、社員版ポータルでは特に重要となる。
 */
(function () {
  'use strict';
  ...
  // GASから取得できない場合はポータル本体を止めず、通知だけを非表示にする。
  // タスク通知は補助機能として扱い、業務画面の利用可能性と切り離している。
